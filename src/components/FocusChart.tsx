...
export const FocusChart: React.FC<FocusChartProps> = ({
      ...
      <div className="w-full p-4 sm:p-6 rounded-3xl glass-panel border border-white/10 flex flex-col">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="h-5 text-xs text-indigo-200 font-medium">
            {hoveredWeekItem ? (
              <span>
                {hoveredWeekItem.fullDate.toLocaleDateString('en-US', { weekday: 'long' })}: {' '}
                <strong className="text-white">{hoveredWeekItem.pomodoros} Pomodoros</strong> ({formatDuration(hoveredWeekItem.minutes)})
              </span>
            ) : (
              <span className="text-white/40">Hover over a day for details</span>
            )}
          </div>
        </div>
          ...
          {weeklyStats.map((item) => {
            ...
            return (
              <div
                ...
                tabIndex={0}
                aria-label={`${item.dayName}: ${item.pomodoros} pomodoros, ${formatDuration(item.minutes)} focused`}
              >
                {/* Bar Element */}
  ...
  return (
    <div className="w-full p-4 sm:p-6 rounded-3xl glass-panel border border-white/10 flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-white/70">
          Monthly Focus Distribution
        </h3>
        <div className="h-5 text-xs text-indigo-200 font-medium">
          {hoveredMonthItem ? (
            <span>
              {hoveredMonthItem.fullDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}: {' '}
              <strong className="text-white">{hoveredMonthItem.pomodoros} Pomodoros</strong> ({formatDuration(hoveredMonthItem.minutes)})
            </span>
          ) : (
            <span className="text-white/40">Hover over a date for details</span>
          )}
        </div>
      </div>
